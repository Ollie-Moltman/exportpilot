    ...
    try:
        ...
        db.execute('''
            CREATE TABLE IF NOT EXISTS analytics (
                ...
                event_type TEXT, page TEXT, referrer TEXT, country TEXT, user_agent TEXT, created_at TEXT
            )
        ''')
        db.execute(
            'INSERT INTO analytics (event_type, page, referrer, country, user_agent, created_at) VALUES (?, ?, ?, ?, ?, ?)',
            (data.get('event','pageview'), data.get('page',''), data.get('referrer',''),
             data.get('country','unknown'), data.get('ua',''), datetime.now().isoformat())
        )
        db.commit()
...
def api_health():
    return jsonify({'status': 'running', 'version': '1.5.0', 'build': '2026-07-11', 'features': ['HS Classification v40+', 'FTA Rates DB', 'Document Generator (Invoice + Packing List + COO) | Pricing + Lead Capture']})
...
def api_lead():
    ...
    try:
        db = get_db()
        db.execute('CREATE TABLE IF NOT EXISTS leads (id INTEGER PRIMARY KEY, name TEXT, email TEXT, product TEXT, source TEXT, created_at TEXT)')
        db.execute('INSERT INTO leads (name, email, product, source, created_at) VALUES (?, ?, ?, ?, ?)',
            (data.get('name',''), data.get('email',''), data.get('product',''), data.get('source','unknown'), datetime.now().isoformat()))
        db.commit()
